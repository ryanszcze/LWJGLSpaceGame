...
public class Leveltwo extends Scene {
	...
	private AudioManager levelmusic;
	private Audio sound;
	private Audio modStream;

	public Leveltwo(List<Entity> projectiles, List<Entity> eProjectiles, List<Enemy> enemyShips) {
	...
	public void waveManager(int enemyCount, MouseSprite e) {
		for (int i = 0; i < enemyCount; i++) {
			enemyShips.add(new Enemy(150, 100 + 500 * i, 100, eProjectiles, e.getRectangle(), 1, projectiles, 3000, 20,
					currTime, e, projSpeed));
			totalEnemies++;
			if (enemyCount % 9 == 0 || newRow == true) {
				totalEnemies = totalEnemies + 2;
				newRow = true;
				enemyShips.add(new Enemy(50, 500 * i, 500, eProjectiles, e.getRectangle(), 1, projectiles, 3000, 20,
						currTime, e, projSpeed));
				enemyShips.add(new Enemy(50, 300 * i, 500, eProjectiles, e.getRectangle(), 1, projectiles, 3000, 20,
						currTime, e, projSpeed));
			}
		}
	}
	...
	public boolean drawFrame(float delta) {
		...
		try {
			levelmusic.getInstance().loadSample("backgroundmusic", "res/jazz.wav");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		// levelmusic.getInstance().play("backgroundmusic", 100);

		newRow = false;
		boolean start = true;
		Mouse.setGrabbed(true);
		// score=0;
		double rateOfFire = 3000;
		...
		Background2 back = new Background2(Display.getWidth());
		...
		testTime = time;
		try {
			modStream = AudioLoader.getAudio("WAV", ResourceLoader.getResourceAsStream("res/jazz.wav"));
		...
		while (!Display.isCloseRequested()) {
			...
			if (game.getHealth() <= 0) {
				...
				if (testTime2 - testTime > 3000) {
					...
					// AudioManager.getInstance().play("backgroundmusic", 0);
					modStream.stop();
					return false;
				}
			...
			if (start == false) {
				TextureImpl.bindNone();
				other.drawString(Display.getWidth() * 3 / 8, Display.getHeight() * 1 / 2,
